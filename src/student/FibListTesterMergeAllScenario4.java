 ...
 */
public class FibListTesterMergeAllScenario4 {

    public static void main(String[] args) throws FileNotFoundException {
        System.out.println("testing FibList mergeAll scenario 4:");
        System.out.println("(3 or more rows that all fit into underfull top row)");
        ...
        String order = "pyfgcrlaoeuidhtnsqjkxbmwvzpyfgcrlaoeuidhtnsqjkxbmwvzpyf";  //55 items
        for (int i = 0; i < order.length(); i++){
            String s = order.substring(i, i+1);
            fibList.add(s);
        }
        String[] a = new String[fibList.size()];
        fibList.toArray(a);  // to cause it to call mergeAll
        
        // insert another 25
        order  = "aaaaaoooooeeeeeuuuuuiiiii";
        for (int i = 0; i < order.length(); i++){
           String s = order.substring(i, i+1);
           fibList.add(s);
        }
